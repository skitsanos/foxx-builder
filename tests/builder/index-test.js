...
const foxxBuilder = require('../index-refactored');

// Mock the required dependencies for testing
const mockFs = {
    exists: (path) => true,
    list: (path) => ['get.js', 'post.js', 'subdir'],
...
const mockRouter = {
    get: (path, handler) => ({ path, handler, methods: ['get'] }),
    post: (path, handler) => ({ path, handler, methods: ['post'] })
};
...
global.require = (path) => {
    if (path.endsWith('/context-extensions')) {
        return { extension1: true, extension2: () => {} };
    }
    if (path.endsWith('.js')) {
        return {
            handler: (req, res) => res.send({ success: true }),
            params: {
                path: { id: { schema: { type: 'string' }, description: 'User ID' } },
                query: { filter: { schema: { type: 'string' }, description: 'Filter query' } }
            },
            headers: {
                'accept-language': { schema: { type: 'string' }, description: 'Preferred language' }
            },
            body: {
                model: { type: 'object' },
...
function testFoxxBuilder() {
    ...
    try {
        ...
        foxxBuilder.assignParams('path', { id: { schema: { type: 'string' }, description: 'User ID' } }, endpoint);
